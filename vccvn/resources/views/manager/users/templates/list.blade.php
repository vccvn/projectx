                    ...
                    <table class="table table-bordered table-striped">
                        <thead>
                            <tr>
                                ...
                                <th class="text-center min-100">Thời hạn</th>
                                <th>Website</th>
                                <th>Trạng thái</th>
                        ...
                        <tbody>
                            @foreach ($results as $user)
                                ...
                                @php
                                    $domain = $user->domain?$user->domain:($user->alias_domain?$user->alias_domain:($user->subdomain . '.' . $user->base_domain));
                                    $url = 'http' . ($user->ssl?'s':'').'://' . $domain;
                                @endphp
                            <tr class="tr_user" id="crazy-item-{{$user->id}}" data-name="{{$user->name}}">
                                ...
                                <td class="text-center min-100 expired-at">{{$user->expired_at}}</td>
                                <td>
                                    @if (!$user->user_owner_id)
                                        <a target="_blank" href="{{$url}}">Xem</a>
                                    @endif
                                </td>
                                ...
                                <td class="text-center min-100">
                                    ...
                                    @if ($list_type != 'trash')
                                        ...
                                        <a href="javascript:void(0)" data-toggle="m-tooltip" data-placement="left" title data-original-title="Gia hạn" data-id="{{$user->id}}" data-expired="{{$user->expired_at}}" data-domain="{{$user->subdomain . '.' . $user->domain}}" data-name="{{$user->name}}" class="text-primary btn btn-outline-primary btn-sm m-btn m-btn--icon m-btn--icon-only m-btn--pill m-btn--air btn-extension">
