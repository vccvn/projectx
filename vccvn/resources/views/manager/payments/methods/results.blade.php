                        ...
                        <div class="dropdown-menu" x-placement="bottom-start">
                            @if (is_array($method_options))
                                @foreach ($method_options as $value => $text)
                                    <a class="dropdown-item btn-create-payment" href="javascript:void(0);"
                                        data-method="{{ $value }}">{{ $text }}</a>
                                @endforeach
                            @endif
    ...
    <div class="row crazy-list" id="m_sortable_portlets">
        <div class="col-lg-12">
            @foreach ($results as $item)
                ...
                @endphp
                <!--begin::Portlet-->
                <div class="m-portlet method-item m-portlet--collapsed m-portlet--head-sm m-portlet--sortable"
                    data-name="{{ $name = $item->name ? $item->name : (isset($method_options[$method->method]) ? $method_options[$method->method] : $method->method) }}"
                    m-portlet="true" id="crazy-item-{{ $item->id }}" data-id="{{ $item->id }}">
                    <div class="m-portlet__head">
                        <div class="m-portlet__head-caption">
                            <div class="m-portlet__head-title">
                            ...
                            <ul class="m-portlet__nav">
                                ...
                                <li class="m-portlet__nav-item">
                                    <a href="javascript:void(0);" data-id="{{ $item->id }}"
                                        class="m-portlet__nav-link m-portlet__nav-link--icon {{ $btn_class }}">
                    ...
                    <div class="m-portlet__body">
                        <form method="post" action="{{ route($route_name_prefix . 'payments.methods.ajax.save') }}"
                            class="m-form update-payment-method-form">
                            ...
                            <div class="form-group m-form__group ml-0 mr-0">
                                ...
                                <input type="text" name="name" class="form-control m-input m-input--air"
                                    id="{{ $item->id }}" value="{{ $name }}"
                                    placeholder="{{ isset($methodData['inputs']['name']['placeholder']) ? $methodData['inputs']['name']['placeholder'] : 'Viết gì đó' }}">
                            </div>
                            ...
                            @if (isset($methodData['inputs']['guide']))
                                <div class="form-group m-form__group">
                                    ...
                                    <textarea class="form-control m-input m-input--air" name="guide"
                                        id="guide-{{ $item->id }}" rows="3">{{ $item->guide }}</textarea>
                                </div>
