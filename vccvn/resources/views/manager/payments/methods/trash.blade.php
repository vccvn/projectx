                            ...
                            <table class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    @foreach ($results as $method)
                                        ...
                                        <tr class="tr_method" id="crazy-item-{{ $method->id }}"
                                            data-name="{{ $method->name }}">
                                            <td class="check-col">
                                                <label class="m-checkbox m-checkbox--solid m-checkbox--success">
                                                    <input type="checkbox" name="ids[]" value="{{ $method->id }}"
                                                        data-id="{{ $method->id }}" class="crazy-check-item">
                                                    <span></span>
                                                </label>
                                            </td>
                                            <td>
                                                <a href="{{ $edit = 'javascript:void(0);' }}" class="btn-view-detail"
                                                    data-id="{{ $method->id }}">
                                                    {{ $method->name }}
                                                </a>
                                            </td>
                                            <td>
                                                <a style="font-weight:500" href="{{ $edit }}"
                                                    class="btn-view-detail" data-id="{{ $method->id }}">
                                                    {{ isset($method_options[$method->method]) ? $method_options[$method->method] : $method->method }}
                                                </a>
                                            </td>
                                            <td>{{ $method->description }}</td>
                                            <td>
                                                <a href="{{ $edit }}" class="btn-view-detail"
                                                    data-id="{{ $method->id }}">
                                                    Xem
                                                </a>
                                            </td>
                                            <td class="text-center min-100">
                                                <a data-toggle="m-tooltip" data-placement="left" title
                                                    data-original-title="Sửa" href="{{ $edit }}"
                                                    class="btn-edit-method text-accent btn btn-outline-accent btn-sm m-btn m-btn--icon m-btn--icon-only m-btn--pill m-btn--air"
                                                    data-id="{{ $method->id }}">
                                                ...
                                                @if ($list_type == 'trash')
                                                    ...
                                                    <a href="javascript:void(0);" data-id="{{ $method->id }}"
