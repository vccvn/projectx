...
?>
					...
					<div {!! $input->attrsToStr() !!} >
						...
						<div class="form-data d-none" style="display:none">{!! ($fd = $input->hiddenData('create-form-data'))?json_encode($fd):'' !!}</div>
						<div class="tag-list-group form-control m-input ">
							<ul class="tag-list">
								{!! $items !!}
								<li class="input-search-group">
									<input type="search" name="tag_search" class="input-search" autocomplete="off" placeholder="{{$input->placeholder?$input->placeholder:'Nhập từ khóa...'}}">
								</li>
							</ul>
