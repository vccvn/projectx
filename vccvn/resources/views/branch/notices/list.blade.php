...
@section('module.name', $title = 'Thông báo')
                    ...
                    <table class="table table-bordered table-striped">
                        ...
                        <tbody>
                            ...
                            <tr class="tr_user" id="crazy-item-{{$item->id}}" data-name="{{$item->name}}">
                                ...
                                <th>{{$item->id}}</th>
                                <td><a style="font-weight:500" href="#">{{$item->title}}</a></td>
                                <td>{{ $item->type }}</td>
                                <td>{{ $item->description }}</td>
                                <td class="text-center min-100">
                                    {{-- <a data-toggle="m-tooltip" data-placement="left" title data-original-title="Sửa" href="{{route('job.update', ['id'=>$item->id])}}" class="text-accent btn btn-outline-accent btn-sm m-btn m-btn--icon m-btn--icon-only m-btn--pill m-btn--air">
	...
	<script>
		window.crazyItemsInit = function () {
			App.items.init({
				...
				title: @json($title),
				urls:{
					delete_url: @json(route('notices.delete'))
				}
			})
		};
		// khai báo ở dây
