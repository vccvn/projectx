...
@section('content')
	...
	admin_action_menu([
		[
			'url' => route($route_name_prefix.'crawlers.frames.trash'),
			'text' => 'Frame đã xoa',
			'icon' => 'fa fa-trash'
		]
	]);
	...
	<script>
		window.crazyItemsInit = function () {
			App.items.init({
				...
				title:"{{$title}}",
				urls:{
					move_to_trash_url: @json(route($route_name_prefix.'crawlers.frames.move-to-trash'))
				}
			})
		};
		// khai báo ở dây
