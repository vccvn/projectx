            ...
            <div class="m-portlet__head-tools">
                <ul class="m-portlet__nav">
                    <li class="m-portlet__nav-item">
                        <a data-original-title="Thêm task" href="{{ route($route_name_prefix . 'products.crawlers.tasks.create') }}" data-toggle="m-tooltip" data-placement="left" title class="ml-3 btn btn-outline-primary m-btn m-btn--icon btn-sm m-btn--icon-only m-btn--pill m-btn--air"><i class="fa fa-plus"></i></a>
                    </li>
                </ul>
            </div>
        </div>
                                    ...
                                    @foreach ($results as $item)
                                        ...
                                        <tr class="tr_user" id="crazy-item-{{ $item->id }}" data-name="{{ $item->task_url }}">
                                            ...
                                            <td class="min-100 max-250">
                                                <a href="{{ $edit = route($route_name_prefix . 'products.crawlers.tasks.update', ['id' => $item->id]) }}">
                                                    {{ $item->task_url }}
                                                </a>
                                            </td>
    ...
    <script>
        window.crazyItemsInit = function() {
            App.items.init({
                ...
                title: @json($title),
                urls: {
                    delete_url: @json(route($route_name_prefix . 'products.crawlers.tasks.delete'))
                }
            })
        };
        window.crawlerInit = function() {
            App.crawler.init({
                urls: {
                    run_url: @json(route($route_name_prefix . 'products.crawlers.tasks.run')),
                    change_status_url: @json(route($route_name_prefix . 'products.crawlers.tasks.status'))
