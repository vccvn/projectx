...
Crazy.str = {
    ...
    replace: function() {
        ...
        if (this.isSN(a[1])) {
            ...
            if (t >= 3 && Crazy.getType(a[2]) == 'array') {
                str = this.replaceByArr(str, a[1], a[2]);
            } else if (t >= 3 && Crazy.getType(a[2]) == 'string') {
                var obj = {},
                    val = a[2];
                var d = a[1];
                for (var k in d) {
...
Crazy.date = function(format, offset){
    if(!offset) offset = 0;
    var d = new Date();
    var t = {};
...
Crazy.fn = function(){
    ...
    this.getFuncPath = function(fn){
        if(!fn) return null;
        var t = Crazy.getType(fn);
        var f = '';
        if(t=='function'){
            f = 'fn';
        }else if(t == 'string'){
            var a = Crazy.str.replace(fn,' ','');
            if(!a) return null;
            
            if(Crazy.getType(Crazy.getEl(this,a))=='function'){
                f = '_this.'+a;
            }
            else if(Crazy.getType(Crazy.getEl(Crazy,a))=='function'){
                f = 'Crazy.'+a;
            }
            else{
                ...
                if(func_paths.length<2){
                    ...
                    if(typeof window[kk] == 'object'){
                        ...
                        if(Crazy.getType(Crazy.getEl(window[kk],kz))=='function'){
                            f = a;
                        }
                    }
                }
    ...
    this.call = function(fn,args,time){
        ...
        if(this.timeout_status && time){
            if(Crazy.getType(time)=='number' && time > 0){
