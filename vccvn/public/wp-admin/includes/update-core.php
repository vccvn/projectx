...
function update_core( $from, $to ) {
	...
	if ( function_exists( 'get_core_checksums' ) ) {
		...
		if ( is_array( $checksums ) ) {
			foreach ( $checksums as $file => $checksum ) {
				if ( WP_USER_CONTENT === substr( $file, 0, strlen(WP_USER_CONTENT) ) ) {
					continue;
				}
	...
	// Copy languages.
	if ( ! is_wp_error( $result ) && $wp_filesystem->is_dir( $from . $distro . WP_USER_CONTENT . '/languages' ) ) {
		...
		if ( @is_dir( $lang_dir ) ) {
			...
			if ( $wp_lang_dir ) {
				$result = copy_dir( $from . $distro . WP_USER_CONTENT . '/languages/', $wp_lang_dir );
	...
	if ( ! is_wp_error( $result )
		...
		foreach ( (array) $_new_bundled_files as $file => $introduced_version ) {
			...
			if ( $development_build || version_compare( $introduced_version, $old_wp_version, '>' ) ) {
				...
				if ( ! $wp_filesystem->exists( $from . $distro . WP_USER_CONTENT . '/' . $file ) ) {
					continue;
				}
				...
				if ( ! $directory ) {
					...
					if ( ! $wp_filesystem->copy( $from . $distro . WP_USER_CONTENT . '/' . $file, $dest . $filename, FS_CHMOD_FILE ) ) {
					...
					$_result = copy_dir( $from . $distro . WP_USER_CONTENT . '/' . $file, $dest . $filename );
