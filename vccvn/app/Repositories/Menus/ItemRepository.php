...
class ItemRepository extends BaseRepository
{
    ...
    public function getInputGroup()
    {
        ...
        if($owner->web_type == 'ecommerce'){
            // thuong mai dien tu
            $groups['product_category'] = $formGroup['product_category'];
        }
        elseif(in_array($owner->web_type, ['personal', 'business'])){
            // trang gioi thieu
        ...
        if(count($dynamics = get_dynamics(['use_category' => 1]))){
    ...
    public function getRefData($type, $props, $id = 0)
    {
        ...
        $types = ['page','dynamic', 'post_category', 'product_category', 'project_category'];
        $text = $props->text;
