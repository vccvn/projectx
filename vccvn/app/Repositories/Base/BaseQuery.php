...
/**
 * @property \App\Models\Model _model
 */
trait BaseQuery
{
                ...
                if (substr($k, 0, 1) == '@') {
                    ...
                    switch ($f) {
                        ...
                        default:
                            ...
                            }elseif (in_array($eager2 = substr($ff, 0, 9), ['withCount', 'loadCount'])) {
                                $this->eager($eager2, substr($ff, 9), $vl);
                            } elseif (in_array($eager = substr($ff, 0, 4), ['with', 'load'])) {
    ...
    /**
     * thêm diều kiện trùng khớp với ít nhất một trong hai hoặc nhiều diều kiện dầu vào
     *
     * @param array $condition
     * @return static
     */
    public function matchIn($condition = [])
    {
        $prefix = '';
        if ($pre = $this->getTable()) {
            $prefix = $pre . '.';
        ...
        if (count($condition) >= 1) {
            $this->where(function ($query) use ($prefix, $fields, $condition) {
                $i = 0;
                foreach ($condition as $field => $value) {
                    ...
                    if (!$hasPrefix) {
                        // nếu không có prefix và ko có trong fillable thì bỏ qua
                        if (isset($this->whereable) && array_key_exists($field, $this->whereable)) {
                            $f = $this->whereable[$$field];
                        } 
                        elseif (!in_array($field, $fields) && $field != 'id') continue;
                        else $f = $prefix . $field;
                    }
                    if($i == 0){
                        $query->where($f, $value);
                    }else{
                        $query->orWhere($f, $value);
                    }
                    $i++;
                }
            });
