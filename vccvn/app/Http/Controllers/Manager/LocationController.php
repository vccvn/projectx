...
class LocationController extends ManagerController
{
    ...
    public function getRegions()
    {
        ...
        foreach ($provinces as $key => $province) {
            $data[] = $this->regionRepository->createDataIfNotExists($province);
        }
    ...
    public function getDistricts(Request $request)
    {
        if($region = $this->regionRepository->find($request->region_id)){
            ...
            foreach ($districts as $key => $district) {
                $d = $this->districtRepository->createDataIfNotExists($district, $region->id);
            }
            return '
    ...
    public function getWards(Request $request)
    {
        if($district = $this->districtRepository->find($request->district_id)){
            ...
            foreach ($wards as $key => $ward) {
                $d = $this->wardRepository->createDataIfNotExists($ward, $district->id);
            }
            return '
    ...
    public function aaa()
    {
        ...
        if(count($provinces)){
            foreach ($provinces as $id => $name) {
                ...
                $p[] = $this->repository->createDataIfNotExists(compact('name', 'position'));
            }
        }
        if(count($districts)){
            foreach ($districts as $id => $data) {
                $d[] = $this->districtRepository->createDataIfNotExists($data);
            }
        }
