...
class BasePost extends Model
{
    public $table = 'posts';
    public $fillable = [
        'owner_id', 'author_id', 'dynamic_id', 'parent_id', 'category_id', 'category_map',
    ...
    {
        return $this->belongsToMany(File::class, 'file_refs', 'ref_id', 'file_id')->wherePivot('ref', $this->ref);
    }
    ...
    public function getImage($size = null)
    {
        if (!$size) {
            ...
            elseif(preg_match('/[0-9]+x[0-9]+/si', $size)){
                ...
                return asset($p . '/' . $size . '/' . $feature_image);
            }
            elseif(file_exists($p = $fd . '/' . $feature_image)){
                return asset($p);
            }
