...
class WPerValidator extends BaseValidator
{
    public function extends()
    {
        ...
        $this->addRule('check_subdomain', function($attr, $value){
            ...
            $manager = ['webmaster', 'master', 'webadmin', 'admin', 'administrator', 'manager', 'quantri', 'trangquantri', 'cpanel', 'poly', 'fpoly', 'mju'];
        ...
        $this->addRule('check_web_type', function($attr, $value){
            ...
            return in_array(strtolower($value), ['default', 'blog', 'news', 'personal', 'bessiness', 'ecommerce', 'wordpress','vcchosting']);
        });

        $this->addRule('check_expired', function($attr, $value){
            if(!$value) return true;
            if(!parse_date_time($value)) return false;
            return true;
        });
    }
    /**
