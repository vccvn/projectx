...
if(!function_exists('get_content_path')){
    ...
    function get_content_path($f = null, $ownor_id = 0){
        $path = 'static/users/' . get_secret_id($ownor_id) . ($f ? '/' . ltrim($f, '/') : '');
        return $path;
    }
}
...
if(!function_exists('get_owner_home_url')){
    /**
     * lấy dường dẩn sản phẩm bài viết
     * @param string
     * @return string
     */
    function get_owner_home_url($path = null)
    {
        $user = get_owner_id()?get_owner():request()->user();
        if($user){
            $webSetting = $user->userWebSetting;
            if($webSetting->alias_domain) return 'http://'.$webSetting->alias_domain;
            return 'http://'.$webSetting->subdomain.'.'.$webSetting->base_domain;
        }
        return '';
    }
}
