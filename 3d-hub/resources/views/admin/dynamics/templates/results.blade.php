        ...
        <div class="m-portlet__head-tools">
            <ul class="m-portlet__nav">
                <li class="m-portlet__nav-item">
                    <a href="{{route($route_name_prefix.'dynamics.create')}}" data-toggle="m-tooltip" data-placement="left" title data-original-title="Thêm mục đang bài" class="ml-3 btn btn-outline-primary m-btn m-btn--icon btn-sm m-btn--icon-only m-btn--pill m-btn--air"><i class="fa fa-plus"></i></a>
                </li>
            </ul>
        </div>
        @endif
                    ...
                    <table class="table table-bordered table-striped">
                        <thead>
                            <tr>
                                ...
                                <th>Ảnh</th>
                                <th class="min-160 max-250">Tên mục</th>
                                <th>Loại tin bài</th>
                        ...
                        <tbody>
                            ...
                            <tr class="tr_user" id="crazy-item-{{$item->id}}" data-name="{{$item->name}}">
                                ...
                                <td>
                                    <a style="font-weight:500" href="{{$u = route($route_name_prefix.'dynamics.update', ['id'=>$item->id])}}">
                                        <img src="{{$item->getFeatureImage()}}" class="image-thumbnail" alt="{{$item->name}}">
                                    </a>
                                </td>
                                <td class="min-160 max-250"><a href="{{$u}}">{{$item->name}}</a></td>
                                <td>{{ isset($types[$item->post_type])?$types[$item->post_type]:"" }}</td>
                                <td class="min-160 max-250">{{ $item->description }}</td>
                                <td>{{ $item->post_count??0 }}</td>
                                <td class="min-100 actions">
                                    
                                    <a data-original-title="Sửa" href="{{$u}}" data-toggle="m-tooltip" data-placement="left" title class="text-accent btn btn-outline-accent btn-sm m-btn m-btn--icon m-btn--icon-only m-btn--pill m-btn--air">
