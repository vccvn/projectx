        ...
        <div class="m-portlet__head-tools">
            <ul class="m-portlet__nav">
                <li class="m-portlet__nav-item">
                    <a href="{{admin_dynamic_url('categories.create')}}" data-toggle="m-tooltip" data-placement="left" title data-original-title="Thêm danh mục" class="ml-3 btn btn-outline-primary m-btn m-btn--icon btn-sm m-btn--icon-only m-btn--pill m-btn--air"><i class="fa fa-plus"></i></a>
                </li>
            </ul>
        </div>
        @endif
                    ...
                    <table class="table table-bordered table-striped">
                        ...
                        <tbody>
                            ...
                            <tr id="crazy-item-{{$item->id}}" data-name="{{$item->name}}">
                                ...
                                <td><a style="font-weight:500" href="#">{{$item->name}}</a></td>
                                <td>{{ $item->parent?$item->parent->name:'không' }}</td>
                                <td class="max-250" class="min-160">{{ $item->getShortDesc(120) }}</td>
                                <td>{{ $item->post_count??0 }}</td>
                                <td class="min-100 actions">

                                    
                                    <a data-toggle="m-tooltip" data-placement="left" title data-original-title="Sửa" href="{{admin_dynamic_url('categories.update', ['id'=>$item->id])}}" class="text-accent btn btn-outline-accent btn-sm m-btn m-btn--icon m-btn--icon-only m-btn--pill m-btn--air">
