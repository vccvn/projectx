...
<div {!! $wrapper->attrsToStr() !!}>
        ...
        @if (count($gallery))
            @foreach ($gallery as $file)
                ...
                <div class="dz-preview dz-image-preview" id="input-gallery-image-{{$file->id}}">
                    <div class="dz-image">
                        <img data-dz-thumbnail="" alt="{{$file->original_filename}}" src="{{$u = $file->getUrl()}}">
                    </div>
                    <div class="dz-details" data-name="{{$file->original_filename}}" data-extension="{{$file->extension}}" data-url="{{$u}}" data-size="{{$sizeinfo['size']}}{{$sizeinfo['size_unit']}}">
    ...
    <div class="drag-and-drop text-center">
        <label for="{{$input->id}}">Click hoặc kéo thả file vào đây để tải ảnh</label>
    </div>
    ...
    <textarea class="template d-none">
        <div class="dz-preview dz-image-preview" id="input-gallery-image-{$id}">
            ...
            <div class="dz-details"  data-name="{original_filename}" data-extension="{$extension}" data-url="{$url}">
