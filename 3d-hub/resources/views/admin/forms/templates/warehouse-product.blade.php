    ...
    <div class="product-items">
        @if ($maxIndex)
            @foreach ($items as $index => $item)
                ...
                @if (isset($item['product_id']) && $item['product_id'] && $param = get_order_product_item_input($item['product_id'], is_array($item['attr_values'])?$item['attr_values']:[]))
                    <?php $nextIndex = $index+1; ?>
                    @include('admin.forms.templates.order-item', 
                        array_merge($param, [
                            'name' => $baseName,
                            'index' => $index,
