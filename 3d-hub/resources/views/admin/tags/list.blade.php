        ...
        <div class="m-portlet__head-tools">
            <ul class="m-portlet__nav">
                <li class="m-portlet__nav-item">
                    <a href="javascript:void(0);" data-toggle="m-tooltip" data-placement="left" title data-original-title="Thêm quyền mới" class="ml-3 btn-create-tag btn btn-outline-primary m-btn m-btn--icon btn-sm m-btn--icon-only m-btn--pill m-btn--air"><i class="fa fa-plus"></i></a>
                </li>
            </ul>
        </div>
    </div>
                    ...
                    <table class="table table-bordered table-striped list-center">
                        ...
                        <tbody class="crazy-list-body">
                            ...
                            <tr class="tr_user" id="crazy-item-{{$item->id}}" data-name="{{$item->name}}">
                                ...
                                <td><a href="javascript:void(0);" class="data-name">{{$item->name}}</a></td>
                                <td>{{ $item->tag_count??0 }}</td>
                                <td class="text-center min-100">
                                    <a data-toggle="m-tooltip" data-placement="left" title data-original-title="Sửa" href="javascript:void(0);" data-id="{{$item->id}}" class="text-accent btn-edit-tag btn btn-outline-accent btn-sm m-btn m-btn--icon m-btn--icon-only m-btn--pill m-btn--air">
...
<textarea class="tag-templates d-none">
    <tr class="tr_user" id="crazy-item-{$id}" data-name="{$name}">
        <td class="text-center">
            <label class="m-checkbox m-checkbox--solid m-checkbox--success">
                <input type="checkbox" name="ids[]" value="{$id}" data-id="{$id}" class="crazy-check-item"> 
                <span></span>
            </label>
        </td>
        <td><a href="javascript:void(0);" class="data-name">{$name}</a></td>
        <td>0</td>
        <td class="text-center min-100">
            <a data-toggle="m-tooltip" data-placement="left" title data-original-title="Sửa" href="javascript:void(0);" data-id="{$id}" class="text-accent btn-edit-tag btn btn-outline-accent btn-sm m-btn m-btn--icon m-btn--icon-only m-btn--pill m-btn--air">
                <i class="flaticon-edit-1"></i>
            </a>
	...
	<script>
		window.crazyItemsInit = function () {
			App.items.init({
				...
				title:@json($title),
				urls:{
					delete_url: @json(route($route_name_prefix.'tags.delete'))
				}
			})
		};
		window.tagsInit = function () {
			App.tags.init({
				urls:{
                    update_url: @json(route($route_name_prefix.'tags.update')),
                    create_url: @json(route($route_name_prefix.'tags.create'))
				}
			})
		};
		// khai báo ở dây
...
@section('js')
    ...
    <script src="{{asset('static/manager/js/tags.js')}}"></script>
@endsection
