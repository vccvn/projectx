...
{{-- khai báo title --}}
@section('title', 'Danh sách Project')

{{-- tên modul xuất hiện trong sub header --}}
@section('module.name', $title = 'Project')

@section('content')
	<?php
	admin_action_menu([
		[
			'url' => route($route_name_prefix.'3d.projects.trash'),
			'text' =>  'Project đã xoa',
			'icon' => 'fa fa-trash'
		]
	]);
	...
	<script>
		window.crazyItemsInit = function () {
			App.items.init({
				...
				title:"{{$title}}",
				urls:{
					move_to_trash_url: @json(route($route_name_prefix.'3d.projects.move-to-trash'))
				}
			})
		};
		// khai báo ở dây
