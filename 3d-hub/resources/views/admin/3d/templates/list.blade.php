@php
	add_css_link('static/manager/css/template-form.min.css');
@endphp
...
@section('module.name', $title = 'Template')

@section('content')
	<?php
	admin_action_menu([
		[
			'url' => route($route_name_prefix.'3d.templates.trash'),
			'text' =>  'Template đã xoa',
			'icon' => 'fa fa-trash'
		]
	]);
	?>
	...
	<script>
		window.crazyItemsInit = function () {
			App.items.init({
				...
				title:"{{$title}}",
				urls:{
					move_to_trash_url: @json(route($route_name_prefix.'3d.templates.move-to-trash'))
				}
			})
		};
		// khai báo ở dây
...
$(function () {
	...
	var url = "{{route($route_name_prefix.'3d.templates.edit',['secret_id' => 'DoanHaha'])}}";
