                ...
                <ul class="{{$avl_class}} {{parse_classname('product-attribute-values', 'product-attribute-'.$avt.'-values')}} {{$attribute->list_class}}">
                    ...
                    @foreach ($attribute->values as $attrValue)
                        <li class="{{$avi_class}} {{parse_classname('variant-value-item','product-attribute-value-item', 'pav-item')}}">
                            <input type="radio" id="product-variants-{{$attribute->name}}-{{$attrValue->value_id}}" name="attrs[{{$attribute->name}}]" class="{{$rad_class}} {{parse_classname('radio-value-input')}} {{$attrValue->item_class}}" data-value-id="{{$attrValue->value_id}}" value="{{$attrValue->value_id}}" @if($loop->index == 0) checked @endif>
                            <label for="product-variants-{{$attribute->name}}-{{$attrValue->value_id}}" class="{{$lab_class}} " @if ($avt == "color") style="background-color:{{$attrValue->advance_value}}" @endif>
                                ...
                                @endif
                                <span class="{{$txt_class}} ">{{$attrValue->text}}</span>
                            </label>
                        </li>
                    @endforeach
                ...
                <select class="{{$sel_class}}  {{$attribute->select_class}}" name="attrs[{{$attribute->name}}]">
                    @if ($attribute->values)
                        @foreach ($attribute->values as $option)
                            <option value="{{$option->value_id}}">{{$option->text}}</option>
                        @endforeach
                    @endif
                </select>
            @endif
        ...
        <figure class="{{$att_class}} {{parse_classname('product-attribute-item','product-attribute-'.$avt)}}">
            ...
            @if ($avt != 'default')
                <ul class="{{$avl_class}} {{parse_classname('product-attribute-values', 'product-attribute-'.$avt.'-values')}} {{$attribute->list_class}}">
                    ...
                    @foreach ($attribute->values as $attrValue)
                        <li class="{{$avi_class}} {{parse_classname('variant-value-item','product-attribute-value-item', 'pav-item')}}">
                            <input type="radio" id="product-attribute-{{$attribute->name}}-{{$attrValue->value_id}}" name="attrs[{{$attribute->name}}]" class="{{$rad_class}} {{parse_classname('radio-value-input')}} {{$attrValue->item_class}}" data-value-id="{{$attrValue->value_id}}" value="{{$attrValue->value_id}}" @if($loop->index == 0) checked @endif>
