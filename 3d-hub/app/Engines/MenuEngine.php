...
Class MenuEngine{
            ...
            if($baseData && array_key_exists('data', $baseData)){
                ...
                foreach ($baseData['data'] as $key => $value) {
                    if(substr($key, 0, 1) == '@'){
                        ...
                        if($keyFunc == 'yield'){
                        ...
                        }elseif(in_array($keyFunc, ['include', 'module', 'modules', 'module_list'])){
                            ...
                            elseif(in_array($keyFunc, ['modules', 'module', 'module_list'])){
    ...
    public function checkInclude($data)
    {
        ...
        if(array_has_any($data, ['@include', '@module', '@modules', '@module_list'])){
            // kiểm tra xem có yêu cầu include không?
            // có thì duyệt mảng
            $newData = [];
    ...
    public function get(string $filename)
    {
        ...
        if(count($itemList)){
            foreach ($data as $key => $item) {
                if(in_array($key, $itemList)||(isset($item['parent']) && in_array($item['parent'], $itemList))){
                    ...
                    elseif($key == 'settings'){
                        ...
                        if(in_array($webType, ['ecommerce', 'business'])){
                            $item['submenu']['data'][] = [
                                'text' => 'Thanh toán',
                                'title' => 'Thanh toán',
                                'active_key' => 'payments',
                ...
                elseif($key == 'custom'){
                    if(is_array($item)){
                        foreach ($item as $custom) {
                            if($custom == 'posts'){
                                $a = $this->getPostMenuItems();
                                $menuitems = array_merge($menuitems, $a);
                                $issetPost = true;
                            }else{
