...
class PostController extends ClientController
{
    ...
    public function __construct(PostRepository $PostRepository, DynamicRepository $dynamicRepository, PageRepository $pageRepository, CategoryRepository $categoryRepository)
    {
        ...
        $this->pageRepository->mode('mask')
            ...
            ->addDefaultParam('deleted', 'deleted', 0);
        $this->dynamicRepository->mode('mask')->addDefaultParam('deleted', 'deleted', 0);
        $this->categoryRepository->mode('mask')->addDefaultParam('deleted', 'deleted', 0);

        $this->init();

        $display = get_display_setting();
        $this->perPage = $display ? $display->post_per_page(10) : 10;
        $this->postShowListType = $display ? $display->post_show_list_type('post') : 'post';
    ...
    public function viewPost(Request $request)
    {
        return $this->cacheUrl($request, false, function ($request) {
            ...
            // lay thong tin page
            $parent = $this->cacheTask($request, $pageKey, $this->pageRepository)->detail(['slug' => $slug]);
            if ($parent) {
    ...
    public function viewDynamicPage(Request $request)
    {
        return $this->cacheUrl($request, true, function ($request) {
            ...
            // kiểm tra page
            elseif ($page = $this->cacheTask($request, $key, $this->pageRepository)->detail(['slug' => $slug])) {
                ...
                if (count($results)) {
                    ...
                    $view = 'pages.detail';
                    $data['article'] = $page;
                    // $data['related_pages'] = $related_pages;
                }
