...
class CartController extends ClientController
{
    ...
    public function placeOrder(Request $request)
    {
        ...
        // kiểm tra cart
        $cart = $this->repository->getCartWidthDetails();
        if(!$cart || count($cart->details) == 0) return redirect()->back();
        // validate
        ...
        if($user){
            ...
            elseif(!($cus = $this->customerRepository->findBy('email', $user->email))){
                ...
                $customer = $this->customerRepository->createIfNotExists($customerData);
            }
            ...
            if(!$orderConfirm && $data->payment_method == "0" && $confirmShipping) {
                $orderConfirm = true;
        ...
        if(!$orderConfirm && ($data->payment_method == 1 || $data->payment_method == 2)){
        ...
        elseif ($order->payment_method == 0) {
            ...
            if($this->setting->send_mail_notification && is_email($this->setting->mail_notification)){
                Email::from($mailFrom, $company)
                    ->to($this->setting->mail_notification)
                    ->replyTo($mailFrom, $company)
        ...
        elseif($order->payment_method == 0){
            ...
            return redirect()->route('client.payments.transfer')->with([
                'order_id' => $order->id
            ]);
        }
