...
class Category extends Model
{
    ...
    public $fillable = ['dynamic_id', 'parent_id', 'name', 'type', 'slug', 'keywords', 'description', 'feature_image', 'deleted'];
    ...
    public function getFeatureImage($urlencode=false)
    {
        ...
        $url = url('static/categories/'.$feature_image);
    ...
    public function deleteFeatureImage()
    {
        ...
        if($this->feature_image){
            if(file_exists($path = public_path('static/categories/'.$this->feature_image))){
                unlink($path);
            }
            if(file_exists($path2 = public_path('static/categories/thumbs/'.$this->feature_image))){
                unlink($path2);
            }
        }
    }
    ...
    {
        return $this->hasMany(Crazy3DModelItem::class, 'category_id', 'id')->where('status', 'published');
    }
    ...
    {
        return $this->hasMany(Crazy3DTemplate::class, 'category_id', 'id')->where('status', 'published');
    }
