...
class ThemeRepository extends BaseRepository
{
    ...
    public function createMetaData($id)
    {
        ...
        $data = compact('components', 'options', 'areas', 'layout', 'menus');
    ...
    public function active(int $id)
    {
        ...
        $theme->applyMeta();
        if (app(SettingRepository::class)->updateSettingData('system', ['theme_id' => $theme->id])) {
    ...
    public function checkThemeActiveList($theme_id)
    {
        if ($listStr = $this->metadataReposirory->getMetaMeta('data', 0, 'theme_active_list')) {
            $this->theme_active_list = json_decode($listStr, true);
    ...
    public function firstSetup($theme)
    {
        ...
        if ($theme->options) {
            ...
            // du lieu option
            $option = $optionRepository->createIfNotExists([
                            ...
                            foreach ($comData as $key => $comp) {
                                if (isset($cps[$comp['path']])) {
                                    ...
                                    if (!$isActiveBefore) {
                                        ...
                                        $htmlComponents->createIfNotExists($data);
                                    }
                                }
                            }
                        }
