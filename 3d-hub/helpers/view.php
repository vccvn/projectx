...
if(!function_exists('get_service_form')){
    /**
     * tạo ra một đối tượng dom html form
     * @param array $data dữ liệu input
     * 
     * @return Form
     */
    function get_service_form(array $config = []) : Form
    {
        $a = json_decode(file_get_contents(json_path('clients/forms/service.json')), true);
        $form = new Form([
            'inputs' => $a && isset($a['inputs']) ? $a['inputs'] : [],
            'errors' => request()->session()->get('errors')
        ], $config);
...
if(!function_exists('is_support_template')){
    ...
    function is_support_template($template, $type){
        ...
        $input_templates = [
            ...
            'inputgroup'         => ['checkbox', 'checklist', 'inputgroup', 'textarea', 'text'],
            'js'                 => ['js', 'textarea', 'text'],
            'frontend'           => ['frontend', 'textarea', 'text'],
            'package'            => ['package', 'textarea', 'text'],
            'modelpreview'       => ['modelpreview', 'text'],
            '3dpreview'          => ['3dpreview', 'text'],
