...
function make_json_module($args = [], $module=null, $table=null, $path=null){
    ...
    $filemanager = new Filemanager(base_path('json'));
    if($file = $filemanager->save(trim($path, '/').'/'.$module .'/form.json', Str::jsonVi(json_encode(defaultJson($table))), 'json')){
    ...
    if($file = $filemanager->save(trim($path, '/').'/'.$module .'/list.json', Str::jsonVi(json_encode($json)), 'json')){
