...
class ColumnItem{
        ...
        if(is_array($data)){
            foreach($data as $key => $value){
                if(!is_array($value)){
                    ...
                    if(isset($value['call']) && (method_exists(static::$item, $value['call']) || is_callable('static::'.$value['call']) || is_callable($value['call']))){
                        $c = $value['call'];
                        ...
                        $call = method_exists(static::$item, $c)?[static::$item, $c]:(
                            is_callable('self::'.$c)?'self::'.$c:$c
                        );
                        if($call == 'salf::getData'){
                            $d[$key] = static::getData(...$params);
                        }
                        else{
    ...
    public static function getData($key){
        $key = str_eval($key, static::$item->toArray(), 0, '');
        return static::$config->get('data.'.$key);
    }
    /**
    ...
    public static function getDataFromString($raw)
    {
        ...
        elseif (in_array($s = substr($raw,0,1), [':', '@'])) {
            ...
            elseif(method_exists(static::$item, $nsp)){
                return static::$item->{$nsp}();
            }elseif(is_callable('ColumnItem::'.$nsp)){
                $a = 'ColumnItem::'.$nsp;
                return $a();
