...
abstract class Mask implements Countable, ArrayAccess, IteratorAggregate, JsonSerializable, Jsonable, Arrayable {
    ...
    final protected function setup($model, $collectionClass = null)
    {
        ...
        if(method_exists($this, 'toMask')){
            $this->____data____store____ = $this->toMask($model);
        }elseif(method_exists($model, 'toMask')){
            $this->____data____store____ = $model->toMask();
        }elseif(method_exists($model, 'getAttrData')){
            $this->____data____store____ = $model->getAttrData();
        }elseif(method_exists($model, 'toArray')){
            $this->____data____store____ = $model->toArray();
        }
     ...
     * @return void
     */
    final protected function parseRNewelation($name, $collection)
    {
        if(!$this->model) return [];
        // $relations = $this->model->getRelations();
        if($collection && count($collection)){
            $this->relations[$name] = $this->parseRelation(
                $collection, 
                array_key_exists($name, $this->relationMap) ? $this->relationMap[$name]: null
            );
        }
    ...
    {
        return new ArrayIterator($this->____data____store____);
    }
    ...
    {
        $this->____data____store____[$name] = $value;
    }
    ...
    public function __get($name)
    {
        ...
        if(array_key_exists($name, $this->____data____store____)){
            return $this->____data____store____[$name];
        }
    ...
    {
        return isset($this->____data____store____[$key]) ? true : (
    ...
    {
        return count($this->____data____store____);
    }

     
    public function offsetSet($offset, $value) {
        if (is_null($offset)) {
            $this->____data____store____[] = $value;
        } else {
            $this->____data____store____[$offset] = $value;
        }
    }

    public function offsetExists($offset) {
        return isset($this->____data____store____[$offset]);
    }

    public function offsetUnset($offset) {
        unset($this->____data____store____[$offset]);
    }
    ...
    public function toArray()
    {
        ...
        $a = array_merge($this->____data____store____, $this->getRelationsLoaded());
        if(is_array($this->hidden) && count($this->hidden)){
    ...
    public function __call($name, $arguments)
    {
        ...
        return array_key_exists($name, $this->____data____store____) ? $this->____data____store____store____[$name] : (isset($arguments[0])?$arguments[0]:null);
    }
