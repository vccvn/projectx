(function ($) {
    ...
    var CosticFramework = (function () {
        ...
        var activeMenuItem = function ($item) {
            $item.addClass('active');
            $item.parents('.collapse').prev().addClass('active');
            if ($item.parents('.collapse').length) {
                $item.closest('.collapse').addClass('show');
            }
            if ($item.parent().children('.collapse').length) {
                $item.parent().children('.collapse').addClass('show');
            }
        ...
        function setActiveMenuItem() {
            ...
            var p = location.pathname.split("/").slice(-1)[0].replace(/^\/|\/$/g, '');
            var current = location.href.split("#")[0];
            // console.log(current);
            $('.ms-main-aside .menu-item a', $('#ms-side-nav')).each(function () {
                var $this = $(this);
                if (p === "" || p === "index.html" || p === "/" || p === "home") {
                    //for root url
                    ...
                    if (hh.length < 2) {
                        if (cc.length < 2) {
                            if (href == current || (cc[0]).indexOf(href + "/") !== -1) {//} || (href).indexOf(current+"/") !== -1 || (href).indexOf(current+"\\") !== -1 ){
                                activeMenuItem($this);
                            }
                        } else if (hh[0] == cc[0] || (cc[0]).indexOf(href + "/") !== -1) {
                            activeMenuItem($this);
                        }
                    } else if (cc.length > 2) {
                        if (cc[0] == hh[0]) {
                            var cd = getDataFromSearch(cc[1]);
                            var hd = getDataFromSearch(hh[1]);
                            var s = true;
                            for (const key in hd) {
                    ...
                    // else if (current.indexOf($this.attr('href')) !== -1) {
                    //   activeMenuItem($this)
                    // }
                }
            });
