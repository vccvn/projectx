    ...
    doAction:function(action, method, success){
        ...
        ajaxRequest(url, method, {id:id}, function(rs){
            ...
            if(rs.status){
                ...
                App.modal.success(message, function(){
                    if(typeof success == "function"){
                        success(rs.data);
                    }
                });
            }else{
                App.modal.warning("Đã có lỗi xảy ra vui lòng kiểm tra thông tin và thử lại sau giây lát");
            }
            
        }, function(err){
            hideLoading();
            
            App.modal.error("Lỗi không xác dịnh. Vui lòng thử lại sau giây lát");
...
$(function(){
    if(typeof transaction_service == 'object'){
        Transaction.service.init(transaction_service);
    }
});
