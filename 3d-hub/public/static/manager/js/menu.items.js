$(function(){
    const MenuItem = function(options) {
        ...
        this.selectTypes = ["page", "dynamic", "product_category", "project_category", "post_category"];
        ...
        this.enableSort = function(){
            App.func.call("App.nestable.add", ['#crazy-menu-item-list']);
        };
        this.disableSort = function(){
            App.func.call("App.nestable.remove", ['#crazy-menu-item-list']);
        };
        ...
        this.delete = function(id){
            App.Swal.confirm("bạn có chắc chắn muốn xóa item này?", function(status){
                // if(status){
                    showLoading();
        ...
        this.showFormData = function(data){
            ...
            if(prefix == 'post_category' || prefix == 'post'){
            ...
            if(self.selectTypes.indexOf(data.type) >= 0){
                if(data.type != 'post_category' && data.type != 'post'){
        ...
        this.onStart = function() {
            ...
            $('.update-menu-item-form').on('submit', function(e){
                e.preventDefault();
                var $form = $(this);
                var data = $form.serializeArray();
                self.save(data, function (rs) {
