(function ($) {
    function CrazyTable(options) {
        ...
        this.makeNavTool = function () {
            if (!$el.parent().parent().find('.crazytable-nav-tool').length) {
                $el.parent().after(
                    '<div class="row crazytable-nav-tool">' +
                    '<div class="col-md-auto text-center crazy-tool text-md-left mb-2">' +
            ...
            $navTool = $el.parent().parent().find('.crazytable-nav-tool');
            $tool = $navTool.find('.crazy-tool');
