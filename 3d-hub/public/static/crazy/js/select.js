function CrazySelect(selector, template){
    this.$el = $(selector);
    ...
    this.activeValue = '';
    this.activeText = '';
    this.$message = this.$el.find('.dropdown-menu .message');
    this.$buttons = this.$el.find('.dropdown-menu .buttons');
    this.$options = this.$el.find('.dropdown-menu .option-list');
    this.$text = this.$el.find('.btn.show-text-value');
    ...
    this.init = function(){
        ...
        let cc = $el.data('confirm-change') || false;
        ...
        // tim kiếm
        $el.children('.dropdown-menu').on('keyup mouseup', function (e) {
            var el = e.target;
        ...
        // click option
        $el.find('.dropdown-menu').on('click', function (e) {
            var el = e.target;
            var tag = el.tagName.toLowerCase();
            if(tag != 'a'){
                e.stopPropagation();
                    ...
                    if(value != old){
                        const changeValue = function(){
                            ...
                            if(!datadisable || datadisable == 'false'){
                                ...
                                self.$text.val(value);
                                isl.val(value);
                                $el.find('.dropdown-menu').find('.option-item').removeClass('active');
                                $elm.addClass('active');
                                ...
                                if(oncbs.length>1){
                                    oncbs.forEach(element => {
                                        ...
                                        if(App.func.check(func)){
                                            App.func.call(func,[value,self]);
                                        }
                                    });
                                    
                                }
                                else if(App.func.check(onc)){
                                    App.func.call(onc,[value,self]);
                                }
                            }
                        };
                        if(self.confirmChange){
                            ...
                            App.Swal.confirm(message, function(agree){
                                // if(agree){
                                    changeValue();
                                // }
                            })
                ...
                }else if($elm.hasClass('crazy-select-asvance-button')){
                    App.func.call(self.advanceHandler, [self.$searchInput.val()]);
    ...
    this.staticSearch = function (keyword){
        var cc = 0;
        var $options = self.$el.children('.dropdown-menu');
        ...
        $options.find('.option-list>.option-item').each(function (ind, elem) {
            if(keywords.length){
                ...
                keywords.forEach(text => {
                    if(!isSearch){
                        if(App.str.clearUnicode($(elem).data('text') + "").toLowerCase().split(text).length>1 || App.str.clearUnicode($(elem).html() + "").toLowerCase().split(text).length>1 || App.str.clearUnicode($(elem).data('value') + "").toLowerCase().split(text).length>1){
                            $(elem).show();
                            $(elem).addClass('found');
                            cc++;
                            isSearch = true;
            ...
            else if(App.str.clearUnicode($(elem).data('text') + "").toLowerCase().split(keyword).length>1 || App.str.clearUnicode($(elem).html() + "").toLowerCase().split(keyword).length>1 || App.str.clearUnicode($(elem).data('value')).toLowerCase().split(keyword).length>1){
                $(elem).show();
                $(elem).addClass('found');
                cc++;
            }else{
        ...
        var checkGroup = function(group, callback){
            ...
            if(dropdownHeader.length){
                if(keywords.length){
                    ...
                    keywords.forEach(text => {
                        if(!isSearch){
                            if(App.str.clearUnicode($(dropdownHeader[0]).data('text')).toLowerCase().split(text).length>1 || App.str.clearUnicode($(dropdownHeader[0]).data('value')).toLowerCase().split(text).length>1){
                ...
                else if(App.str.clearUnicode($(dropdownHeader[0]).data('text')).toLowerCase().split(keyword).length>1 || App.str.clearUnicode($(dropdownHeader[0]).data('value')).toLowerCase().split(keyword).length>1){
                    s = true;
                    $(group).find('.option-group').show();
                    $(group).find('.option-item').show();
                    $(group).find('.option-item').addClass('found');
    ...
    this.ajaxSearch = function(keyword, callback){
        ...
        ajaxRequest(this.searchUrl, "GET", data, function(rs){
            if(rs.status){
                if(rs.data){
                    self.hideLoader();
                    // do something
        ...
        var checkGroup = function(group, callback){
            ...
            if(dropdownHeader.length){
                if(keywords.length){
                    ...
                    keywords.forEach(text => {
                        if(!isSearch){
                            if(App.str.clearUnicode($(dropdownHeader[0]).data('text')+"").toLowerCase() == text){
                                isSearch = true;
                                s = true;
                            }
                        }
                ...
                else if(App.str.clearUnicode($(dropdownHeader[0]).data('text')+"").toLowerCase() == keyword){
                    s = true;
                }
            }
            if(!s){
        ...
        for (const key in options) {
            ...
            if(['object', 'array'].indexOf(t) >= 0){
                ...
                if(group_header!='value'){
                    html += "<h6 class=\"dropdown-header\">"+lbl+"</h6>";
                }else{
    ...
    this.parseMultiLevelText = function(){
        ...
        if(items.length){
            ...
            var checkGroup = function(group, callback, level){
                ...
                var dropdownHeader = $(group).children('.dropdown-header');
                if(dropdownHeader.length){
                    if($(dropdownHeader[0]).hasClass('active')){
                        data[level] = $(dropdownHeader[0]).html();
                ...
                if(s){
                    data[level] = dropdownHeader.html();
                    if(level > maxLevel) maxLevel = level;
                }
                return s;
            };
            ...
            $options.find('.option-list>.option-group').each(function (index, elem) {
                var $header = $(elem).children('.dropdown-header');
        ...
        if(maxLevel>=0){
            ...
            self.changeText(dataText.join(" <span class='fa fa-chevron-right'></span> "));
        }
    ...
    this.changeOptions = function(options){
        ...
        self.$input.val('');
        var optHtml = self.getSelectOptions(options, self.oldValue);
        self.$options.html(optHtml);
    ...
    this.deactive = function(){
        this.$el.find('.dropdown-menu').find('.option-item').removeClass('active');
    };
