...
export class LayoutProMenuComponent implements OnInit, OnDestroy {
  ...
  constructor(private menuService: MenuService, private router: Router, public pro: BrandService, private cdr: ChangeDetectorRef) {}
  ...
  private genMenus(data: Menu[]) {
    ...
    this.menuService.visit(ingoreCategores, (item, parent) => {
      if (!item._aclResult) {
        if (this.disabledAcl) {
          item.disabled = true;
        } else {
  ...
  private openStatus() {
    ...
    let item = this.menuService.getHit(this.menus, this.router.url, true);
    if (!item) {
      this.cd();
      return;
    }
  ...
  ngOnInit() {
    ...
    this.menuService.change.pipe(takeUntil(unsubscribe$)).subscribe((res) => this.genMenus(res));
    router.events
      .pipe(
